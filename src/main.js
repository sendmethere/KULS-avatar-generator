...
import { createIcons, Shapes, CircleHelp, FolderOpen, Download, Shuffle, ArrowUpRight, UserRound, Check, Layers3, Undo2, Redo2, Rotate3d, Bone, Box, Camera, Focus, Mouse, Activity, View, Component, Smile, Save, X, ScanFace, Scissors, PersonStanding, Shirt, Pipette, SlidersHorizontal, ScanEye, Sparkles, Mail } from 'lucide';
const icons={Shapes,CircleHelp,FolderOpen,Download,Shuffle,ArrowUpRight,UserRound,Check,Layers3,Undo2,Redo2,Rotate3d,Bone,Box,Camera,Focus,Mouse,Activity,View,Component,Smile,Save,X,ScanFace,Scissors,PersonStanding,Shirt,Pipette,SlidersHorizontal,ScanEye,Sparkles,Mail};
...
const html=`
<header class="topbar"><a class="brand" href="/" aria-label="KULS Avatar Simulator 홈"><span class="brand-icon">${icon('shapes')}</span><span>KULS <span class="brand-light">Avatar Simulator</span><small>작은 조합, 무한한 캐릭터</small></span></a><div class="top-center"><span class="status-dot"></span> 로컬 스튜디오 <span class="divider">/</span> <span id="save-status">자동 저장됨</span></div><div class="header-actions"><button class="icon-btn" id="help" title="사용 안내" aria-label="사용 안내">${icon('circle-help')}</button><button class="language-toggle" id="language-toggle" type="button" aria-label="언어를 English로 변경" title="언어를 English로 변경">EN</button><button class="quiet" id="load">${icon('folder-open')} 불러오기</button><button class="primary" id="export-open">${icon('download')} 내보내기</button></div></header>
...
$('#app').innerHTML=html;
$('#wire').setAttribute('aria-label','와이어프레임');
...
const classroomToolbar=$('.classroom-toolbar'), classroomMenuGroup=document.createElement('div'), classroomContextDescription=$('#classroom-context-description');
classroomMenuGroup.className='classroom-menu-group';
classroomMenuGroup.append($('.classroom-contexts'),$('.classroom-scenarios'));
classroomToolbar.prepend(classroomMenuGroup);
classroomToolbar.after(classroomContextDescription);
const refreshIcons=()=>createIcons({icons,attrs:{'stroke-width':1.7}});
refreshIcons();
function applyLanguage(){
 document.documentElement.lang=locale;
 document.documentElement.querySelector('title').textContent=locale==='en'?'KULS Avatar Simulator · Your 3D character':'KULS Avatar Simulator · 나만의 3D 캐릭터';
...
async function exportGLB(){
 ...
 const a=createAvatar(config,{animations:true,tpose:true});a.pose('tpose');a.setExpression(config.expression,config.intensity,0);a.setGaze(config.gazeX,config.gazeY);a.group.userData={generator:'KULS Avatar Simulator',config:{...config},license:'User-created procedural asset'};
